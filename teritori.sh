...
GITURL=https://github.com/TERITORI/teritori-chain
BRANCH=teritori-testnet-v2
...
cd $HOME
rm teritori -rf
git clone $GITURL --branch $BRANCH
cd teritori-chain
make build
sudo chmod +x ./build/teritorid && sudo mv ./build/teritorid /usr/local/bin/teritorid
...
sed -i -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0uqck\"/" $HOME/.teritorid/config/app.toml
...
[Service]
User=$USER
ExecStart=$(which teritorid) --home $HOME/.teritorid start
Restart=on-failure
RestartSec=3
LimitNOFILE=65535
